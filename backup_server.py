...
def chat_interface(message, history):
    ...
    if HF_TOKEN and has_hf_credits(HF_TOKEN):
        try:
            ...
            result = client.text_generation(prompt, model="gpt2", max_new_tokens=128)
            return result
        except Exception:
            pass
    # Fallback to Ollama
...
if __name__ == "__main__":
    gr.ChatInterface(
        fn=chat_interface,
        title="Backup Inference Server",
        description="Uses Hugging Face if credits are available, otherwise Ollama as backup."
    ).launch(server_name="0.0.0.0", server_port=5000)
